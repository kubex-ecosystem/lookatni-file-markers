...
load_manifest_config() {
  local _root_dir="${_ROOT_DIR:-${ROOT_DIR:-$(git rev-parse --show-toplevel)/cli}}"
...
create_archive() {
  ...
  local _binary_without_suffix="${_binary_name%%_*}"

  if [[ "${_platform}" == "windows" ]]; then
    local _archive_name="${_binary_name%.exe}.zip"
    mv "${_binary_name}" "${_binary_without_suffix}"
    if command -v zip >/dev/null 2>&1; then
      if zip -9 "${_archive_name}" "${_binary_without_suffix}" >/dev/null 2>&1; then
        log success "Created archive: ${_archive_name}"
      else
        log warn "Failed to create ZIP archive for ${_binary_without_suffix}"
      fi
    fi
    ...
    mv "${_binary_name}" "${_binary_without_suffix}"
    if tar -czf "${_archive_name}" "${_binary_without_suffix}" 2>/dev/null; then
      log success "Created archive: ${_archive_name}"
    else
      log warn "Failed to create tar.gz archive for ${_binary_without_suffix}"
    fi
    mv "${_binary_without_suffix}" "${_binary_name}"
  fi
