...
class LookAtniGenerator {
  ...
  private isTextFile(filePath: string): boolean {
    ...
    // BINARY DETECTION: Efficient binary file detection
    ...
    // For files without extension, check for null bytes (binary indicator)
    if (!ext && fs.existsSync(filePath)) {
      try {
        const buffer = fs.readFileSync(filePath, { flag: 'r' });
        if (buffer.length > 0) {
          // Check first 512 bytes for null bytes (common in binaries)
    ...
    // Default: assume it's text if no extension and no binary patterns
    return true;
...
export { GenerateOptions, GenerateStats, LookAtniGenerator };
